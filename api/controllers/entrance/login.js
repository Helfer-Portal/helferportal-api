...
password attempt.`,
  ...
  inputs: {
    identity: {
      description:
        'The email or username to try in this attempt, e.g. "irl@example.com, user1234".',
      type: 'string',
      required: true
    },
...
and exposed as \`req.me\`.)`
  ...
  fn: async function(inputs, exits) {
    const identity = inputs.identity.toLowerCase();
    ...
    // if (!User.isAuthorized(userRecord)) {
    //   throw 'blocked';
    // }

    // If the password doesn't match, then also exit thru "badCombo".
    await sails.helpers.crypto
    ...
    // const performSideload = true;
    // Ember.buildResponse(User, userRecord, userAssociations, performSideload)
    // return exits.success('/api/v3/users/' + userRecord.id);
