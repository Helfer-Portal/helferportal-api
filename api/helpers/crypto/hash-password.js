...
module.exports = {
  ...
  description:
    'Hash a password (i.e. one-way encryption) using the BCrypt algorithm.',

  extendedDescription:
    'The BCrypt algorithm is _one-way_-- in other words, passwords encrypted (aka "hashed") this way ' +
  ...
  inputs: {
    ...
    strength: {
      ...
      example: 12,
      extendedDescription:
        'Strength is measured in this case by the number of iterations it takes to generate the cryptographic key.  Hashes generated with a higher "strength" value will take longer to crack with brute force, but will also take longer to generate.  A minimum of 10 (the default) is recommended.',
      moreInfoUrl: 'https://en.wikipedia.org/wiki/Bcrypt',
      defaultsTo: 10
    }
  },
  ...
  exits: {
    success: {
      ...
      outputExample:
        '$2a$10$VC.etVpgnfYrUt5/TW4ktOy91yv/gWC6c.XXeK7jx69ukP/4Ocgj2',
      outputDescription:
        'The (irreversibly munged) password hash generated from the provided password.',
      extendedDescription:
  ...
  fn: async function(inputs, exits) {
    try {
      const hash = await bcrypt.hash(inputs.password, inputs.strength);
